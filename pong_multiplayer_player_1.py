...
while True:
    ...
    if received:
        if received == "end game":
            ...
            isNextBallOwner = True #kommer til å ha ballen når det starter igjen
            resetBall() #ballen går til sin opprinnelige posisjon (se l. 20-28)
        elif received == "P2 lost": #hvis p2 har tapt
            wins += 1 #får p1 et poeng
            display.show("W")
            sleep(1000)
        ...
        elif received != "P2 lost": #hvis den får egenskapene av ballen til den andre
            isBallOwner = True
            receivedValues = received.split(",") 
            #split blir brukt til å lage en liste av egenskapene og seperere dem hos ','
    ...
    if isBallOwner: #hvis den har ballen
        ...
        if yPos == 0: #hvis ballen er bak spilleren
            losses += 1
            radio.send("P1 lost") #sender at den har tapt
            display.show("L")
            sleep(1000)
            resetBall()
            ...
            if yPosTest >= 5: #hvis den kommer til å være utenfor veggen som er mellom spillerne
                radio.send(str(xPos) + "," + str(yPosTest) + "," + str(deltaX) + "," + str(deltaY)) #sender verdiene til ballen
                isBallOwner = False #har ikke ballen lenger
            else:
